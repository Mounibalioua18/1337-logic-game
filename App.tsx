...
const App: React.FC = () => {
  ...
  // Load overall progress on mount
  useEffect(() => {
  ...
  // When changing levels, load the functions specific to that level
  useEffect(() => {
    ...
    ['F1', 'F2', 'F3', 'F4', 'F5'].forEach((f, i) => {
      ...
      if (parsedSaved && parsedSaved[f]) {
        // Load saved code, but trim/pad to current level limits
    ...
    // Auto-select first slot of F1
    setSelectedSlot({ fn: 'F1', idx: 0 });
  }, [level]);

  // Save progress whenever functions change
  ...
  const run = async () => {
    ...
    const MAX_TOTAL_STEPS = 8000; // Increased for extra hard recursive levels
      ...
      for (const cmd of cmds) {
        ...
        while (true) {
          ...
          if (cmd.action.startsWith('F') && cmd.action.length === 2) {
            ...
            await new Promise(r => setTimeout(r, 40)); // Even faster for complex paths
          ...
          if (iteration > 500) break; // Increased protection for high-recursion
        }
      }
      return false;
    };
  ...
  const handleSpy = () => {
    ...
    if (!solution || Object.keys(solution).length === 0) {
      alert("No pre-stored solution. You must find the logic sequence yourself.");
      return;
    }
    const newFns = { ...functions };
  ...
  const renderIcon = (action: ActionType, size: string = "w-6 h-6") => {
    switch (action) {
      ...
      default: return <span className="text-[12px] font-black">{action}</span>;
    }
  };

  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col font-sans select-none overflow-x-hidden">
      <div className="bg-slate-800 border-b border-slate-700 px-6 py-4 flex items-center justify-between shadow-xl sticky top-0 z-40">
        <div className="flex items-center gap-6">
          <h1 className="text-xl font-black italic tracking-tighter text-blue-400">1337_LOGIC_PRO</h1>
          <div className="hidden md:flex flex-col gap-1">
             <div className="flex justify-between text-[10px] font-bold text-slate-500 uppercase">
                <span>Progress</span>
                <span>{Math.round((unlockedLevel / LEVELS.length) * 100)}%</span>
             </div>
        ...
        <div className="flex items-center gap-4">
          <button onClick={handleSpy} className="text-[10px] bg-slate-700 px-3 py-1.5 rounded-lg border border-slate-600 hover:bg-slate-600 transition font-bold uppercase tracking-wider text-slate-300">Unlock Sol.</button>
          <div className="bg-slate-900 px-4 py-2 rounded-xl border border-slate-700 text-sm font-black">
             LVL <span className="text-blue-400">{level.id}</span>
          </div>
        </div>
      </div>

      <div className="flex-1 flex flex-col p-4 lg:p-10 gap-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-7 flex flex-col gap-6">
            <div className="bg-slate-800 rounded-[2rem] p-6 lg:p-10 shadow-2xl border border-slate-700 flex flex-col items-center relative overflow-hidden">
              ...
              <div 
                className="grid gap-px bg-slate-700 rounded-2xl overflow-hidden shadow-inner border-[6px] border-slate-900"
                style={{ 
              ...
              >
                {level.grid.map((row, r) => row.map((cell, c) => {
                  ...
                  return (
                    <div key={`${r}-${c}`} className={`relative flex items-center justify-center transition-all w-full h-full aspect-square ${CELL_COLORS[cell as CellType]}`}>
                      {hasStar && !isCollected && (
                        <div className="w-1/2 h-1/2 text-yellow-300 animate-pulse drop-shadow-[0_0_8px_rgba(253,224,71,0.6)]">
                      ...
                      {execState.pos[0] === r && execState.pos[1] === c && (
                        ...
                        >
                          <CursorArrow className="w-full h-full drop-shadow-[0_4px_10px_rgba(0,0,0,0.5)]" />
                        </div>
                      )}
                    </div>
                  );
              ...
              <div className="mt-8 text-center text-slate-400 text-sm font-medium max-w-lg leading-relaxed">
                <span className="text-blue-400 font-black uppercase text-[10px] block mb-1">Instruction</span>
          ...
          <div className="lg:col-span-5 flex flex-col gap-8">
            <div className="bg-slate-800 rounded-3xl p-6 border border-slate-700 shadow-xl">
              <h3 className="text-[10px] font-black uppercase text-slate-500 mb-6 tracking-[0.2em] flex items-center gap-2">
                 <div className="w-1.5 h-1.5 rounded-full bg-blue-500"></div> Operations
              </h3>
                ...
                {(['FORWARD', 'LEFT', 'RIGHT', 'F1', 'F2', 'F3', 'F4', 'F5'] as ActionType[]).map(a => {
                  ...
                  return (
                    <button 
                      ...
                      className="aspect-square rounded-2xl bg-slate-900 border-2 border-slate-700 flex items-center justify-center hover:border-blue-500 hover:text-blue-400 transition-all active:scale-90 group shadow-lg"
                    >
                      {renderIcon(a, "w-8 h-8 group-hover:scale-110 transition")}
                    </button>
                  );
                })}
              </div>
              ...
              <h3 className="text-[10px] font-black uppercase text-slate-500 mb-6 tracking-[0.2em] flex items-center gap-2">
                 <div className="w-1.5 h-1.5 rounded-full bg-emerald-500"></div> Conditions
              </h3>
              <div className="flex gap-4">
                {(['RED', 'BLUE', 'GREEN'] as ConditionType[]).map(c => (
                  <button 
                    ...
                    className={`w-14 h-14 rounded-2xl border-4 transition-all transform active:scale-90 shadow-xl
                      ${c === 'RED' ? 'bg-red-500 border-red-400' : c === 'BLUE' ? 'bg-blue-600 border-blue-400' : 'bg-emerald-600 border-emerald-400'}
                      ${selectedSlot && functions[selectedSlot.fn][selectedSlot.idx]?.condition === c ? 'ring-4 ring-white ring-offset-4 ring-offset-slate-800' : 'opacity-70 hover:opacity-100'}`} 
                  />
                ))}
              </div>
            </div>

            <div className="bg-slate-800 rounded-3xl p-6 border border-slate-700 shadow-xl flex-1 overflow-y-auto max-h-[500px]">
              <h3 className="text-[10px] font-black uppercase text-slate-500 mb-6 tracking-[0.2em] flex items-center gap-2">
                 <div className="w-1.5 h-1.5 rounded-full bg-purple-500"></div> Code Grid
              </h3>
              <div className="space-y-8">
                {Object.keys(functions).map((fn, i) => {
                  ...
                  return (
                    <div key={fn} className="flex flex-col gap-3 group">
                      <div className="flex items-center justify-between">
                         <div className="px-3 py-1 rounded-lg bg-slate-900 text-blue-400 font-black text-xs border border-slate-700">
                           {fn}
                         </div>
                         <div className="text-[10px] font-bold text-slate-600 uppercase">{functions[fn].filter(c => c !== null).length} / {limit}</div>
                      </div>
                      <div className="flex flex-wrap gap-2.5 bg-slate-900/50 p-4 rounded-2xl border border-slate-700/50 min-h-[72px]">
                        {functions[fn].map((cmd, idx) => (
                          <div 
                            key={idx}
                            onClick={() => setSelectedSlot({ fn, idx })}
                            onContextMenu={(e) => { e.preventDefault(); clearSlot(fn, idx); }}
                            className={`w-12 h-12 rounded-xl border-2 flex items-center justify-center cursor-pointer transition-all relative flex-shrink-0
                              ${selectedSlot?.fn === fn && selectedSlot?.idx === idx ? 'border-blue-500 bg-blue-500/10 shadow-[0_0_15px_rgba(59,130,246,0.3)] scale-110 z-10' : 'border-slate-700 bg-slate-900 hover:border-slate-500'}
                              ${!cmd ? 'dashed opacity-30' : ''}`}
                          >
                            {cmd && (
                              <div className={`w-full h-full flex items-center justify-center rounded-lg ${cmd.condition === 'RED' ? 'bg-red-500/20' : cmd.condition === 'BLUE' ? 'bg-blue-500/20' : cmd.condition === 'GREEN' ? 'bg-emerald-500/20' : ''}`}>
                                 {renderIcon(cmd.action, "w-7 h-7")}
                                 {cmd.condition && (
                                   <div className={`absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full border-2 border-slate-900 shadow-lg ${cmd.condition === 'RED' ? 'bg-red-500' : cmd.condition === 'BLUE' ? 'bg-blue-500' : 'bg-emerald-500'}`}></div>
                                 )}
                              </div>
                            )}
                          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-slate-800/90 backdrop-blur-xl border-t border-slate-700 p-6 flex items-center justify-center gap-10 z-50">
        <button 
          onClick={() => setCurrentLevelIdx(Math.max(0, currentLevelIdx - 1))} 
          className="w-14 h-14 rounded-2xl bg-slate-900 border border-slate-700 flex items-center justify-center hover:bg-slate-700 transition disabled:opacity-20" 
        ...
        <div className="flex gap-6">
          <button onClick={reset} className="w-16 h-16 rounded-3xl bg-slate-700 border border-slate-600 shadow-xl flex items-center justify-center text-slate-400 hover:text-white transition active:rotate-180 duration-700">
          ...
          <button 
            onClick={run} 
            className={`w-24 h-24 rounded-[2.5rem] shadow-[0_15px_30px_-5px_rgba(0,0,0,0.5)] flex items-center justify-center transition-all transform active:scale-95 relative
              ${execState.status === 'RUNNING' ? 'bg-red-600 ring-8 ring-red-600/20' : 'bg-blue-600 ring-8 ring-blue-600/20 hover:scale-105 hover:bg-blue-500'}`}
          >
            {execState.status === 'RUNNING' 
              ? <div className="w-8 h-8 bg-white rounded-sm animate-pulse"></div> 
              : <svg className="w-12 h-12 ml-2 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>}
          </button>
        </div>
      ...
      {execState.status === 'WON' && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-500">
          <div className="bg-slate-800 border-2 border-blue-500 p-12 rounded-[3rem] shadow-2xl text-center max-w-sm w-full mx-4 flex flex-col items-center gap-8 scale-in-center">
             ...
             <div>
                <h2 className="text-4xl font-black italic tracking-tighter mb-2">COMPLETE</h2>
                <p className="text-slate-400 font-bold text-sm uppercase tracking-widest">Sequence Validated</p>
             </div>
             <button 
                onClick={() => {
                ...
                className="w-full py-4 bg-blue-600 hover:bg-blue-500 transition rounded-2xl font-black text-lg shadow-xl"
             >
                {currentLevelIdx === LEVELS.length - 1 ? 'REPLAY' : 'NEXT MODULE'}
             </button>
          </div>
        </div>
      )}
